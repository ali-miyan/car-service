...
const ServiceInfo = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-center text-2xl font-bold uppercase mb-6">Service Info</h1>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white shadow-md p-6 rounded-lg">
          <h2 className="text-lg font-semibold mb-4 text-center">Company Details</h2>
          <ul className="list-none space-y-2">
            <li><strong>Company Name:</strong> {companyDetails?.companyName}</li>
            <li><strong>Owner Name:</strong> {companyDetails?.ownerName}</li>
            <li><strong>Year:</strong> {companyDetails?.year}</li>
            <li><strong>License Number:</strong> {companyDetails?.licenseNumber}</li>
            <li><strong>License Expiry:</strong> {companyDetails?.licenseExpiry}</li>
            <li><strong>Email:</strong> {companyDetails?.email}</li>
          </ul>
        </div>

        <div className="bg-white shadow-md p-6 rounded-lg">
          <h2 className="text-lg font-semibold mb-4 text-center">Contact and Location</h2>
          <ul className="list-none space-y-2">
            <li><strong>Contact 1:</strong> {companyDetails?.contact1}</li>
            <li><strong>Contact 2:</strong> {companyDetails?.contact2}</li>
            <li><strong>Address:</strong> {companyDetails?.address.address}</li>
            <li><strong>Pin Code:</strong> {companyDetails?.address.postcode}</li>
            <li><strong>Street:</strong> {companyDetails?.address.streetRegion}</li>
            <li><strong>City:</strong> {companyDetails?.address.city}</li>
            <li><strong>Country:</strong> {companyDetails?.address.country}</li>
          </ul>
        </div>
      </div>

      <h2 className="text-center text-xl font-bold uppercase my-6">Licensing Information</h2>
      ...
      {selectedImage && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-75 z-50">
          <div className="relative">
            <img src={selectedImage} alt="Fullscreen" className="w-auto h-screen object-contain" />
            <button
